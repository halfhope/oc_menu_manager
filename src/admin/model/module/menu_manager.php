...
namespace Opencart\Admin\Model\Extension\MenuManager\Module;
class MenuManager extends \Opencart\System\Engine\Model {
	...
	public function recursiveGlob($pattern, $flags = 0) {
		...
		if ($files) {
			...
			foreach ($files as $key => $value) {
				...
				$name = $this->language->get('extension_heading_title');
	...
	public function linkHasPremissions($link) {
		...
		$is_external_link = (strripos($link, HTTP_SERVER) === false) && ( defined('HTTPS_SERVER') && (strripos($link, HTTPS_SERVER) === false));
		if (!$is_external_link) {
