<?php
// Heading
$_['heading_title']     = 'Menu manager | Left menu';

// Text
$_['text_extension']    = 'Extensions';
$_['text_edit']         = 'Edit left menu';

$_['text_edit_item']    = 'Edit';
$_['text_view_item']    = 'View';
$_['text_remove_item']  = 'Delete';
$_['text_add_sub_item'] = 'Add';

$_['text_left_menu']    = 'Current state';
$_['text_preset_menu']  = 'Scanned sections';

// Placeholder
$_['placeholder_name']  = 'Name';
$_['placeholder_link']  = 'Link';
$_['placeholder_icon']  = 'Icon';
$_['placeholder_route'] = 'Route. E.g. catalog/product/edit';

// Modal
$_['modal_title']       = 'Select icon';
$_['modal_search']      = 'Search';

// Button
$_['button_expand']     = 'Expand all';
$_['button_collapse']   = 'Collapse all';
$_['button_add']        = 'Add';
$_['button_reset']      = 'Reset';

// Help
$_['left_menu_help']     = 'In links, you can use the $_GET parameters of the current page. For example, <b>[product_id]</b> will be replaced with $_GET[\'product_id\'] of the current page. <br> <br> You can also use settings (config). For example, <b>[config|config_language_id]</b>.';

// Error
$_['error_permission']  = 'Warning: You do not have permission to modify module!';

// Success
$_['text_success']      = 'Success: You have modified account module!';